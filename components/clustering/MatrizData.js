    ...
    if (props.var !== null) {
      ...
      for (const item in props.var.corr) {
        objectData.push({
          ...
          gender: props.var.corr[item]["Gender"],
          everMarried: props.var.corr[item]["Ever_Married"],
          age: props.var.corr[item]["Age"],
          graduated: props.var.corr[item]["Graduated"],
          profession: props.var.corr[item]["Profession"],
          workExperience: props.var.corr[item]["Work_Experience"],
          spendingScore: props.var.corr[item]["Spending_Score"],
          familySize: props.var.corr[item]["Family_Size"],
          var: props.var.corr[item]["Var_1"]
        });
      }
          ...
          <div>
            ...
            <DataTable value={corrData} rows={6}>
              ...
              <Column
                ...
                body={(data) => fieldColor(data.gender)}
              />
              <Column
                field="everMarried"
                header="Ever_Married"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.everMarried)}
              />
              <Column
                field="age"
                header="Age"
              ...
              <Column
                ...
                header="Graduated"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.graduated)}
              />
              <Column
                field="profession"
                header="Profession"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.profession)}
              />
              <Column
                field="workExperience"
                header="Work_Experience"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.workExperience)}
              />
              <Column
                field="spendingScore"
                header="Spending_Score"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.spendingScore)}
              />
              <Column
                field="familySize"
                header="Family_Size"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.familySize)}
              />
              <Column
                field="var"
                header="Var_1"
                style={{ width: "15%", fontSize: 12 }}
                body={(data) => fieldColor(data.var)}
              />
            </DataTable>
